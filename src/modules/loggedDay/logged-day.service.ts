...
import { DeleteByIdsRequestDto } from '@modules/user/dto';
...
export class LoggedDayService {
  ...
  async findOne(id: string): Promise<LoggedDay> {
    return await this.loggedDayRepository.findById(id);
  }
  ...
  async delete(id: string): Promise<string | void> {
    const loggedDay = await this.findOne(id);
    if (!loggedDay) {
      throw new NotFoundException(`There isn't any loggedDay with id: ${id}`);
    }
    await this.loggedDayRepository.delete(id);
  }

  async deleteByIds(Ids: DeleteByIdsRequestDto): Promise<object[]> {
    const undeletedIds: object[] = [];

    const loggedDays = Ids.ids.map(async (id) => {
    ...
    await Promise.allSettled(loggedDays);
    return undeletedIds;
  }
}
