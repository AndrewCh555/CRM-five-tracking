...
export class LoggedDayController {
  ...
  async delete(@Param('id', ParseUUIDPipe) loggedDayId: string): Promise<void> {
    await this.loggedDaysService.delete(loggedDayId);
  }

  @Delete('byIds')
  @ApiOperation({ description: 'Delete logged days' })
  @HttpCode(HttpStatus.OK)
  @UseGuards(RolesGuard)
  @CheckAbilities({ action: MethodsEnum.Delete, subject: LoggedDay })
  @ApiOkResponse({ description: 'Undeleted ids', type: Object, isArray: true })
  async deleteByIds(@Body() ids: DeleteByIdsRequestDto): Promise<object[]> {
    return await this.loggedDaysService.deleteByIds(ids);
  }
}
