...
export class DepartmentController {
  ...
  @Put(':id')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ description: 'Update by ID department' })
  @UseGuards(RolesGuard)
  @CheckAbilities({ action: MethodsEnum.Update, subject: Department })
  @ApiInternalServerErrorResponse({ description: 'Internal server error' })
  public async update(
    @Param('id', ParseUUIDPipe) departmentId: string,
    @Body() data: CreateDepartmentRequestDto,
  ): Promise<void> {
    await this.departmentsService.update(data, departmentId);
  }
