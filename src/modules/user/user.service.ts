...
export class UserService {
  ...
  async findByEmailForToken(email: string): Promise<User> {
    const user = await this.userRepository.findOneEmail(email);
  ...
  async findOneWithToken(email: string, refreshToken: string): Promise<User> {
    return await this.userRepository.findOneWithToken(email, refreshToken);
  }
  ...
  async updateUser(id: string): Promise<void> {
    ...
    await this.userRepository.updateUser(id, role.id);
  }
